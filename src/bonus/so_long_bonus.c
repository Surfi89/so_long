...
void	ft_window(t_game *game)
{
	...
	game->mlx_win = mlx_new_window(game->mlx,
			game->pxl * game->map_x,
			game->pxl * game->map_y, "Solongmon: Version 42");
	ft_fill_window(game);
}
...
void	ft_check_pce(char *buffer, t_game *game)
{
	...
	{
		printf("Error\nIncorrect number of Players, PkBalls or Exits.\n");
		exit(1);
	}
}
...
{
	printf("Error!\nNot enough trees in the bordering forest.\n");
...
{
	game->file = mlx_xpm_file_to_image(game->mlx,
			"./img/Coinf.xpm", &game->pxl, &game->pxl);
	mlx_put_image_to_window (game->mlx, game->mlx_win,
		game->file, game->pxl * game->x,
		game->pxl * game->y);
	game->collect++;
...
void	ft_put_exit(t_game *game)
{
	if (game->collect == 0)
		game->file = mlx_xpm_file_to_image(game->mlx,
				"./img/exit2.xpm", &game->pxl, &game->pxl);
	else
		game->file = mlx_xpm_file_to_image(game->mlx,
				"./img/exit3.xpm", &game->pxl, &game->pxl);
	mlx_put_image_to_window (game->mlx, game->mlx_win,
		game->file, game->pxl * game->x,
		game->pxl * game->y);
}
...
int	ft_keyboard(int keyhook, t_game *game)
{
	...
	if (game->collect == 0
		...
		ft_close_window(game);
		printf("Pf.Oak: Thank you for collecting all my Pokemon!\n");
...
void	ft_enemy(t_game *game)
{
	...
	printf("Pf.Oak: Failed! You cannot catch Mewtwo... \n");
	ft_close_window(game);
}